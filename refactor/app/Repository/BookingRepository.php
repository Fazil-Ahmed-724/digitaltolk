...
class BookingRepository extends BaseRepository
{
    ...
    public function __construct(Job $model, MailerInterface $mailer)
    {
        ...
        $this->logger->pushHandler(
            new StreamHandler(storage_path('logs/admin/laravel-'
                . date('Y-m-d') . '.log'),
                Logger::DEBUG)
        );
    ...
    public function getUsersJobs($userId)
    {
        ...
        if ($jobs) {
            ...
            $noramlJobs = collect($noramlJobs)->each(function ($item) use ($userId) {
    ...
    public function getUsersJobsHistory($userId, Request $request)
    {
        ...
        $noramlJobs = array();
        if ($cuser && $cuser->is('customer')) {
            $jobs = $cuser->jobs()->with('user.userMeta',
                'user.average',
                'translatorJobRel.user.average',
                'language',
                'feedback',
                'distance')->whereIn('status',
                [
                    'completed',
                    'withdrawbefore24',
                    'withdrawafter24',
    ...
    public function jobEnd($postData = array())
    {
        ...
        $name = $user->name;
        $subject = 'Information om avslutad tolkning för bokningsnummer # '
        . $job->id;
        $sessionExplode = explode(':', $job->session_time);
        $sessionTime = $sessionExplode[0]
            . ' tim ' . $sessionExplode[1] . ' min';
        $data = [
            'user' => $user,
            'job' => $job,
    ...
    public function sendNotificationTranslator($job, $data = [], $excludeUserId)
    {
        ...
        foreach ($users as $oneUser) {
            if ($oneUser->user_type == '2'
                && $oneUser->status == '1'
                && $oneUser->id != $excludeUserId) { // user is translator and he is not disabled
        ...
        $logger->pushHandler(
            new StreamHandler(storage_path('logs/push/laravel-'
                . date('Y-m-d') . '.log'),
                Logger::DEBUG)
        );
        $logger->pushHandler(new FirePHPHandler());
        $logger->addInfo('Push send for job '
            . $job->id,
            [
                $translatorArray, $delpayTranslatorArray, $msgText,
                $data,
            ]
        );
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        foreach ($translators as $translator) {
            ...
            Log::info('Send SMS to ' . $translator->email
                . ' (' . $translator->mobile . '), status: '
                . print_r($status, true)
            );
        }
...
//        foreach ($jobIds as $k => $v)     // checking translator town
    ...
    private function changeTimedoutStatus($job, $data, $changedTranslator)
    {
        ...
        if ($data['status'] == 'pending') {
            ...
            $job->save();
            $subject = 'Bekräftelse - tolk har accepterat er bokning (bokning # '
            . $job->id . ')';
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout', 'assigned'])) {
        ...
        if ($data['status'] == 'assigned' && $changedTranslator) {
            ...
            $this->mailer->send($translator->email,
                $translator->name,
                $subject, 'emails.job-changed-translator-new-translator', $dataEmail);
            ...
            $this->mailer->send($email, $name, $subject,
    ...
    public function sendSessionStartRemindNotification($user, $job, $language, $due, $duration)
    {
        ...
        if ($job->customer_physical_type == 'yes') {
            $msgText = array(
                ...
                . $language . 'tolkning (på plats i ' . $job->town .
                ') kl ' . $dueExplode[1] . ' på ' . $dueExplode[0] .
                ' som vara i ' . $duration .
            ...
            $msgText = array(
                "en" => 'Detta är en påminnelse om att du har en '
                . $language . 'tolkning (telefon) kl '
                . $dueExplode[1] . ' på ' . $dueExplode[0] .
                ' som vara i ' . $duration .
                ' min.Lycka till och kom ihåg att ge feedback efter utförd tolkning!',
            );
        }
    ...
    private function changeAssignedStatus($job, $data)
    {
        if (in_array($data['status'], ['withdrawbefore24', 'withdrawafter24', 'timedout'])) {
            ...
            if (in_array($data['status'], ['withdrawbefore24', 'withdrawafter24'])) {
                ...
                $subject = 'Information om avslutad tolkning för bokningsnummer #' . $job->id;
                $this->mailer->send($email, $name, $subject,
                    'emails.status-changed-from-pending-or-assigned-customer', $dataEmail
                );
    ...
    private function changeTranslator($currentTranslator, $data, $job)
    {
        ...
        if (!is_null($currentTranslator) || (isset($data['translator'])
            && $data['translator'] != 0) || $data['translator_email'] != '') {
            $logData = [];
            if (!is_null($currentTranslator)
                && ((isset($data['translator'])
                    && $currentTranslator->user_id != $data['translator'])
            ...
            } elseif (is_null($currentTranslator) && isset($data['translator'])
                && ($data['translator'] != 0 || $data['translator_email'] != '')) {
                if ($data['translator_email'] != '') {
                    $data['translator'] = User::where('email', $data['translator_email'])->first()->id;
                }
            ...
            if ($translatorChanged) {
                return [
                    ...
                    'new_translator' => $newTranslator, 'log_data' => $logData,
                ];
            }
    ...
    public function sendExpiredNotification($job, $user)
    {
        ...
        $language = TeHelper::fetchLanguageFromJobId($job->from_language_id);
        $msgText = array(
            "en" => 'Tyvärr har ingen tolk accepterat er bokning: ('
            . $language . ', ' . $job->duration . 'min, ' . $job->due . '). Vänligen pröva boka om tiden.',
        );
    ...
    private function sendNotificationChangePending($user, $job, $language, $due, $duration)
    {
        ...
        if ($job->customer_physical_type == 'yes') {
            $msgText = array(
                "en" => 'Du har nu fått platstolkningen för '
                . $language . ' kl ' . $duration . ' den ' . $due .
                '. Vänligen säkerställ att du är förberedd för den tiden. Tack!',
            );
        } else {
            $msgText = array(
                "en" => 'Du har nu fått telefontolkningen för '
                . $language . ' kl ' . $duration . ' den ' . $due .
                '. Vänligen säkerställ att du är förberedd för den tiden. Tack!',
            );
        }
    ...
    public function acceptJobWithId($jobId, $cuser)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($jobId, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $jobId)) {
                ...
                $response['message'] = 'Du har nu accepterat och fått bokningen för '
                . $language . 'tolk ' . $job->duration . 'min ' . $job->due;
            } else {
                // Booking already accepted by someone else
                $language = TeHelper::fetchLanguageFromJobId($job->from_language_id);
                $response['status'] = 'fail';
                $response['message'] = 'Denna ' . $language . 'tolkning '
                . $job->duration . 'min '
            ...
            $response['status'] = 'fail';
            $response['message'] = 'Du har redan en bokning den tiden '
            . $job->due . '. Du har inte fått denna tolkning';
        }
        return $response;
    }
    ...
    public function cancelJobAjax($data, $user)
    {
        ...
        if cancelation is within 24 hours - translator will
        be informed AND the customer will get an addition to
        his number of bookings - so we will charge of it if
        the cancelation is within 24 hours
        ...
        if ($cuser->is('customer')) {
            ...
            if ($translator) {
                ...
                $language = TeHelper::fetchLanguageFromJobId($job->from_language_id);
                $msgText = array(
                    "en" => 'Kunden har avbokat bokningen för '
                    . $language . 'tolk, ' . $job->duration . 'min, '
            ...
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    ...
                    $language = TeHelper::fetchLanguageFromJobId($job->from_language_id);
                    $msgText = array(
                        "en" => 'Er ' . $language . 'tolk, ' . $job->duration .
                        'min ' . $job->due .
            ...
            if (isset($requestdata['translator_email']) && count($requestdata['translator_email'])) {
                ...
                if ($users) {
                    $allJobIDs = DB::table('translator_job_rel')
                        ->whereNull('cancel_at')->whereIn('user_id', collect($users)
                            ->pluck('id')->all())->lists('job_id');
